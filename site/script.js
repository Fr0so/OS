...
const orangePanels = {
  ...
  camp: () => `
    <div class="camp-pack-layout">
      <div class="camp-task-card"><span>Task</span><strong>Lukewarm beer lead</strong><small>Assigned to Name</small></div>
      <div class="packing-list-panel">
        ...
        <label><input type="checkbox"> Dry socks hidden from everyone</label>
      </div>
    </div>`,
  access: () => `
...
function openFlightDetail(flightId) {
  if (flightLocked && flightId !== confirmedFlightId) return;
...
function updateFlightOverviewLock() {
  document.querySelectorAll(".flight-route-button").forEach((button) => {
    ...
    button.classList.toggle("unavailable-flight", flightLocked && !isSelected);
    button.disabled = flightLocked && !isSelected;
  });
}
...
function updateOrangeLineupForDay(day) { selectedOrangeDay = day; renderOrangePanel("lineup"); }
function confirmOrangeAllocation() { queueMsnMessage("CAMPEN", `Welcome ${USER_NAME}. Your Orange week access is noted for ${selectedOrangeDay}. Camp handoff is active, and yes — you are in charge of lukewarm beer for the week.`); }
...
function renderMsnContacts() {
  ...
  if (!entries.length) {
    ...
    empty.className = "msn-contact-empty";
    empty.textContent = "No contacts yet.";
    msnContactList.appendChild(empty);
    return;
  }
