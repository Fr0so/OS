...
const campItems = [["tape", "Pavilion tape"], ["powerbank", "Powerbank"], ["rain", "Rain cover"], ["sunscreen", "Sunscreen"], ["socks", "Dry socks"], ["beer", "Borrowing beer for the week"]];
...
let booted = false, highestZIndex = 10, currentComputerView = "root", selectedTrashId = null, travelLoaded = false;
let currentTravelName = DEFAULT_USER_NAME, browserHistory = ["home"], selectedFlightId = null, selectedSeat = null;
let selectedOrangeDay = "Mon 29", currentOrangePanel = "lineup", campConfirmed = false, selectedVehicle = null, vehicleReserved = false;
let activeMsnContact = null, draggingWindow = null, dragOffsetX = 0, dragOffsetY = 0, selecting = false, selectionStartX = 0, selectionStartY = 0;
const heldFlightSeats = {};
const msnStore = {};

function showScreen(screenId) { document.querySelectorAll(".screen").forEach((screen) => screen.classList.remove("active")); $(screenId)?.classList.add("active"); }
...
function syncDynamicNames() { document.querySelectorAll(".dynamic-booking-name").forEach((el) => { el.textContent = getBookingName(); }); }
function updateClock() { const clock = $("clock"); if (clock) clock.textContent = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }); }
function shakeElement(element) {
  if (!element) return;
  const base = getComputedStyle(element).transform;
  const prefix = base && base !== "none" ? base + " " : "";
  element.animate([{ transform: `${prefix}translateX(0)` }, { transform: `${prefix}translateX(-7px)` }, { transform: `${prefix}translateX(7px)` }, { transform: `${prefix}translateX(-4px)` }, { transform: `${prefix}translateX(4px)` }, { transform: `${prefix}translateX(0)` }], { duration: 210 });
}
function openPasswordDialog() {
  showScreen("password-screen");
  const password = $("password"), msg = $("login-message");
  if (msg) msg.textContent = "";
  if (password) { password.value = ""; setTimeout(() => password.focus(), 30); }
}
function osLogin() {
  const password = $("password"), msg = $("login-message");
  if (!password || !msg) return;
  if (password.value === OS_PASSWORD) { msg.textContent = ""; startOsLoader(); return; }
  msg.textContent = "Wrong password."; password.value = ""; password.focus(); shakeElement(document.querySelector(".dialog-window"));
}
function startOsLoader() {
  ...
  function next() {
    ...
    index += 1;
    if (index < steps.length) setTimeout(next, 420);
    else setTimeout(() => { showScreen("desktop"); syncDynamicNames(); updateClock(); }, 260);
  }
  next();
}
function openWindow(windowId) {
  const win = $(windowId); if (!win) return;
  highestZIndex += 1; win.classList.add("open"); win.style.zIndex = highestZIndex; placeWindow(win); closeStartMenu();
  ...
  const input = win.querySelector("input:not([readonly])"); if (input) setTimeout(() => input.focus(), 40);
}
function closeWindow(win) { win?.classList.remove("open"); }
function placeWindow(win) { if (!win.style.left) win.style.left = (win.dataset.defaultLeft || "120") + "px"; if (!win.style.top) win.style.top = (win.dataset.defaultTop || "80") + "px"; requestAnimationFrame(() => clampWindow(win)); }
function clampWindow(win) {
  const taskbarHeight = 38, margin = 12, rect = win.getBoundingClientRect();
...
function switchComputerView(view) {
  ...
  const backButton = $("computer-back-button"); if (backButton) backButton.disabled = view === "root";
  const title = view === "root" ? "My Computer" : view === "localdisk" ? "Local Disk (C:) - My Computer" : "Summer 2026 (F:) - My Computer";
  if ($("computer-title")) $("computer-title").textContent = title;
}
function selectTrashFile(id) {
  selectedTrashId = id;
  document.querySelectorAll("#recycle-files .file-item").forEach((item) => item.classList.toggle("selected", item.dataset.trashId === id));
}
...
function restoreRecycleBin() {
  ...
  if (!selectedTrashId) { if (status) status.textContent = "Select one item to restore."; shakeElement($("recycle-window")); return; }
  const map = { passwd: ["passwd-window", "passwd.txt", "passwd-file"], denmark: ["denmark-rant-window", "Danmark VM 26.txt", "denmark-rant-trash"], duke: ["duke-rant-window", "March madness 26.txt", "duke-rant-trash"] };
...
function emptyRecycleBin() { const status = $("recycle-status"); if (status) status.textContent = "Permission denied."; shakeElement($("recycle-window")); }
function handlePasswdInBin() { selectTrashFile("passwd"); const status = $("recycle-status"); if (status) status.textContent = "This item is locked."; shakeElement($("recycle-window")); }
function runTravelLogin() {
  const username = $("travel-username-input")?.value.trim() || "", password = $("travel-password-input")?.value.trim() || "", msg = $("travel-login-message");
  if (username.toLowerCase() === TRAVEL_USERNAME && password.toLowerCase() === TRAVEL_PASSWORD) {
    currentTravelName = username || getOsName(); syncDynamicNames(); if (msg) msg.textContent = ""; $("travel-login-screen")?.classList.add("hidden");
    if (travelLoaded) { $("travel-loader-screen")?.classList.add("hidden"); $("travel-browser")?.classList.remove("hidden"); navigateBrowser("home", false); } else startTravelLoader();
    return;
  }
  if (msg) msg.textContent = "Authentication failed.";
  const passInput = $("travel-password-input"); if (passInput) { passInput.value = ""; passInput.focus(); }
  shakeElement($("program-window"));
}
function startTravelLoader() {
  const loader = $("travel-loader-screen"), browser = $("travel-browser"), progress = $("progress-bar"), line = $("loader-line"), log = $("loader-log");
  if (!loader || !browser || !progress || !line || !log) return;
  loader.classList.remove("hidden"); browser.classList.add("hidden"); progress.style.width = "0%"; log.innerHTML = "";
  ...
  function next() {
    ...
    progress.style.width = percent + "%"; line.textContent = text;
    const li = document.createElement("li"); li.textContent = label; log.appendChild(li);
    index += 1;
    if (index < steps.length) setTimeout(next, 360);
...
function navigateBrowser(page, addHistory = true, detail = "") {
  ...
  const address = $("browser-address"); if (address) address.textContent = addressForPage(page, detail);
  const historyItem = detail ? `${page}:${detail}` : page;
  if (addHistory && browserHistory[browserHistory.length - 1] !== historyItem) browserHistory.push(historyItem);
  const backButton = $("browser-back-button"); if (backButton) backButton.disabled = browserHistory.length <= 1;
}
function navigateFromHistory(item) {
  ...
  if (page === "balkan-country") { const country = Object.keys(countryData).find((name) => countryData[name].slug === detail); if (country) setCountryDetail(country, false); navigateBrowser(page, false, detail); return; }
  navigateBrowser(page, false);
}
function browserBack() { if (browserHistory.length <= 1) return; browserHistory.pop(); navigateFromHistory(browserHistory[browserHistory.length - 1]); }
...
function renderFlights() {
  ...
  Object.entries(flightData).forEach(([id, flight]) => {
    ...
    button.className = "flight-card"; if (isHeld) button.classList.add("held");
    button.type = "button"; button.dataset.flightId = id;
    button.innerHTML = `<div class="flight-card-top"><strong>${flight.route}</strong><span>${flight.number}</span></div><div class="flight-meta"><span>${flight.departCode}</span><span>${flight.departTime}</span><span>${flight.stop}</span><span>${flight.arriveTime}</span><span>${flight.arriveCode}</span></div><div class="flight-footer"><span>${flight.duration}</span><em>${isHeld ? "Held" : "View"}</em></div>`;
    button.addEventListener("click", () => openFlightDetail(id)); grid.appendChild(button);
  });
}
function openFlightDetail(id) { setFlightDetail(id); navigateBrowser("flight-detail", true, id); }
function setFlightDetail(id) {
  const flight = flightData[id]; if (!flight) return;
  selectedFlightId = id; selectedSeat = Object.prototype.hasOwnProperty.call(heldFlightSeats, id) ? (heldFlightSeats[id] || null) : null;
  $("flight-detail-route").textContent = flight.route; $("flight-detail-number").textContent = flight.number;
  ...
  $("flight-detail-duration").textContent = flight.duration; $("flight-detail-fare").textContent = flight.fare; $("flight-detail-cabin").textContent = flight.cabin; $("flight-detail-baggage").textContent = flight.baggage;
  renderSeatMap(); updateFlightDetailState();
}
function renderSeatMap() {
  const grid = $("seat-grid"); if (!grid) return; grid.innerHTML = "";
  seats.forEach((seat) => { const button = document.createElement("button"); button.className = "seat-button"; button.type = "button"; button.dataset.seat = seat; button.textContent = seat; button.addEventListener("click", () => selectSeat(seat)); grid.appendChild(button); });
}
function updateFlightDetailState() {
  const holdButton = $("hold-flight-button"), seatCard = $("seat-card"), status = $("seat-status"), seatCopy = $("flight-detail-seat-copy");
  ...
  document.querySelectorAll(".seat-button").forEach((button) => { const seat = button.dataset.seat; button.classList.toggle("active", !!selectedSeat && selectedSeat === seat); button.disabled = isHeldFlight; });
  seatCard?.classList.toggle("locked", isHeldFlight);
  if (!isHeldFlight) { holdButton.disabled = false; holdButton.textContent = "Hold this option"; status.textContent = selectedSeat ? `Seat selected: ${selectedSeat}` : "No seat selected."; seatCopy.textContent = "Available"; return; }
  holdButton.disabled = true; holdButton.textContent = "Held"; status.textContent = selectedSeat ? `Held seat: ${selectedSeat}` : "Held without seat."; seatCopy.textContent = "Locked";
}
function selectSeat(seat) { if (Object.prototype.hasOwnProperty.call(heldFlightSeats, selectedFlightId)) return; selectedSeat = seat; document.querySelectorAll(".seat-button").forEach((button) => button.classList.toggle("active", button.dataset.seat === seat)); const status = $("seat-status"); if (status) status.textContent = `Seat selected: ${seat}`; }
function holdSelectedFlight() {
  ...
  const flight = flightData[selectedFlightId]; heldFlightSeats[selectedFlightId] = selectedSeat || "";
  renderFlights(); updateFlightDetailState();
  queueMsnMessage(flight.desk, `Hi ${getBookingName()}. ${flight.number} (${flight.route}) is now on hold. ${selectedSeat ? `Seat ${selectedSeat} noted.` : "Seat not selected."}`);
}
function renderOrangeDays() {
  const ribbon = $("orange-day-ribbon"); if (!ribbon) return; ribbon.innerHTML = "";
  orangeDays.forEach((day) => { const button = document.createElement("button"); button.className = "day-pill"; button.classList.toggle("active", selectedOrangeDay === day); button.type = "button"; button.dataset.day = day; button.textContent = day; button.addEventListener("click", () => { selectedOrangeDay = day; renderOrangeDays(); if (currentOrangePanel === "lineup") renderOrangePanel("lineup"); }); ribbon.appendChild(button); });
}
function renderOrangePanel(panel) {
  currentOrangePanel = panel;
  document.querySelectorAll(".orange-tab").forEach((button) => button.classList.toggle("active", button.dataset.orangePanel === panel));
  const container = $("orange-panel"); if (!container) return;
  if (panel === "lineup") {
    const file = orangeLineup[selectedOrangeDay];
    container.innerHTML = `<div class="lineup-board"><div class="lineup-board-head"><span>${selectedOrangeDay}</span><div><strong>${file.board}</strong><div class="lineup-meta-strip">${file.meta.map((item) => `<b>${item}</b>`).join("")}</div></div></div><div class="lineup-rows">${file.picks.map(([name, tag], index) => `<div class="lineup-row ${index === 0 ? "top-row" : ""}"><em>${String(index + 1).padStart(2, "0")}</em><strong>${name}</strong><span>${tag}</span></div>`).join("")}</div></div>`;
    updateOrangeConfirmState(); return;
  }
  if (panel === "camp") {
    container.innerHTML = `<div class="camp-layout"><div class="camp-task-card"><span>Camp task</span><strong>Borrowing beer for the week</strong><small>${getBookingName()}</small><div class="camp-progress-bar"><span class="camp-progress-fill" id="camp-progress-fill"></span></div><em class="camp-progress-text" id="camp-progress-text">0 / ${campItems.length} packed</em></div><div class="packing-list" id="camp-packing-list"><h3>Packing list</h3>${campItems.map(([key, label]) => `<label data-camp-label="${key}"><input class="camp-checkbox" data-camp-key="${key}" type="checkbox" ${campChecklist[key] ? "checked" : ""} ${campConfirmed ? "disabled" : ""} />${label}</label>`).join("")}</div></div>`;
    bindCampChecks(); updateOrangeConfirmState(); return;
  }
  const allPacked = Object.values(campChecklist).every(Boolean);
  container.innerHTML = `<div class="access-sheet"><div class="access-sheet-title"><span>Access note</span><strong>${getBookingName()} / row 017</strong></div><div class="access-note-grid"><div class="access-note-card"><span>Wristband</span><strong>on-site handoff</strong></div><div class="access-note-card"><span>Camp</span><strong>${allPacked ? "ready for confirm" : "packing pending"}</strong></div><div class="access-note-card"><span>Bring</span><strong>ID + ticket mail</strong></div><div class="access-note-card"><span>Window</span><strong>partner desk</strong></div></div><div class="access-fineprint">orange_row=017 // camp_file=${allPacked ? "ready" : "pending"} // name=${getBookingName()}</div></div>`;
...
function bindCampChecks() {
  document.querySelectorAll(".camp-checkbox").forEach((box) => {
    ...
    box.addEventListener("change", () => { campChecklist[key] = box.checked; label?.classList.toggle("done", box.checked); updateOrangeConfirmState(); });
  });
}
function updateOrangeConfirmState() {
  const total = campItems.length, done = Object.values(campChecklist).filter(Boolean).length, complete = done === total, button = $("orange-confirm-button"), miniStatus = $("orange-camp-mini-status");
  ...
  if (!button) return;
  if (campConfirmed) { button.disabled = true; button.textContent = "Camp confirmed"; return; }
  button.disabled = !complete; button.textContent = complete ? "Confirm camp allocation" : "Complete packing list";
}
function confirmOrange() {
  const complete = Object.values(campChecklist).every(Boolean);
  if (!complete || campConfirmed) { shakeElement($("orange-panel")); return; }
...
function renderCountries() {
  ...
  Object.entries(countryData).forEach(([country, data], index) => {
    const button = document.createElement("button"); button.className = "country-card"; button.type = "button"; button.dataset.country = country;
    button.innerHTML = `<span class="country-card-index">${String(index + 1).padStart(2, "0")} / ${data.code}</span><strong>${country}</strong><small>${data.sub}</small><em>${data.see}</em>`;
    button.addEventListener("click", () => openCountry(country)); grid.appendChild(button);
  });
}
function openCountry(country) { setCountryDetail(country, true); }
function setCountryDetail(country, pushHistory = true) {
  const data = countryData[country]; if (!data) return;
  $("country-path").textContent = `TravelPortal / Balkan / ${data.slug}`; $("country-title").textContent = country; $("country-copy").textContent = data.copy; $("country-code").textContent = data.code; $("country-region").textContent = data.sub; $("country-dish").textContent = data.dish; $("country-see").textContent = data.see; $("country-note").textContent = data.note; $("country-border").textContent = data.border; $("country-drive").textContent = data.drive;
  const itinerary = $("country-itinerary"); if (itinerary) itinerary.innerHTML = `<span>Route marks</span>${data.stops.map((stop) => `<b>${stop}</b>`).join("")}`;
  document.querySelectorAll(".country-card").forEach((button) => button.classList.toggle("active", button.dataset.country === country));
...
function renderVehicles() {
  ...
  Object.entries(vehicleData).forEach(([name, data]) => {
    const button = document.createElement("button"); button.className = "vehicle-card"; button.classList.toggle("active", selectedVehicle === name); button.type = "button"; button.disabled = vehicleReserved; button.dataset.vehicle = name; button.innerHTML = `<strong>${name}</strong><span>${data.sub}</span>`; button.addEventListener("click", () => selectVehicle(name)); grid.appendChild(button);
  });
}
function selectVehicle(name) { if (vehicleReserved) return; selectedVehicle = name; const status = $("rental-status"); if (status) status.textContent = `${name} selected.`; renderVehicles(); }
function reserveVehicle() {
  if (vehicleReserved) return;
  if (!selectedVehicle) { const status = $("rental-status"); if (status) status.textContent = "Choose one vehicle first."; shakeElement(document.querySelector(".rental-desk")); return; }
  vehicleReserved = true; $("reserve-car-button").disabled = true; $("reserve-car-button").textContent = "Vehicle reserved"; $("rental-status").textContent = `${selectedVehicle} reserved.`; renderVehicles();
...
function pushMsnMessage(contact, message, status) {
  ...
  const convo = msnStore[contact]; convo.status = status; convo.messages.push({ text: message, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) });
  const msnOpen = $("msn-window")?.classList.contains("open");
  if (msnOpen && activeMsnContact === contact) showMsnConversation(contact, true); else convo.unread += 1;
...
function renderMsnContacts() {
  ...
  Object.entries(msnStore).forEach(([name, convo]) => {
    const button = document.createElement("button"); button.className = "msn-contact-item"; button.classList.toggle("active", name === activeMsnContact); button.type = "button"; button.dataset.contact = name;
    ...
    button.addEventListener("click", () => showMsnConversation(name, true)); list.appendChild(button);
  });
}
function showMsnConversation(contact, markRead = true) {
  const convo = msnStore[contact]; if (!convo) return;
  activeMsnContact = contact; $("msn-avatar").textContent = contact.slice(0, 1); $("msn-contact-name").textContent = contact; $("msn-contact-status").textContent = convo.status;
  const messages = $("msn-messages"); messages.innerHTML = "";
  convo.messages.forEach((msg) => { const bubble = document.createElement("div"); bubble.className = "msn-message"; bubble.innerHTML = `<small>${msg.time}</small><div>${msg.text}</div>`; messages.appendChild(bubble); });
  if (markRead) convo.unread = 0;
  renderMsnContacts(); updateMsnBadge(); messages.scrollTop = messages.scrollHeight;
}
function updateMsnBadge() {
  ...
  badge.textContent = String(totalUnread); badge.classList.toggle("hidden", totalUnread === 0);
}
...
function bindWindowOpeners() {
  document.querySelectorAll("[data-window]").forEach((button) => {
    button.addEventListener("click", (event) => {
      const isDesktop = button.classList.contains("desktop-icon"), isRecycleFile = button.closest("#recycle-files");
      if (isDesktop) { document.querySelectorAll(".desktop-icon").forEach((icon) => icon.classList.remove("selected")); button.classList.add("selected"); }
      if (isRecycleFile) return;
      event.stopPropagation(); openWindow(button.dataset.window);
    });
  });
}
function bindWindowDragging() {
  document.querySelectorAll(".window-titlebar").forEach((titlebar) => {
    titlebar.addEventListener("mousedown", (event) => {
      ...
      draggingWindow = titlebar.closest(".window"); if (!draggingWindow) return;
      highestZIndex += 1; draggingWindow.style.zIndex = highestZIndex;
      const rect = draggingWindow.getBoundingClientRect(); dragOffsetX = event.clientX - rect.left; dragOffsetY = event.clientY - rect.top;
      document.body.style.cursor = "move"; event.preventDefault();
    });
  });
  document.addEventListener("mousemove", (event) => {
    if (!draggingWindow) return;
    const taskbarHeight = 38, maxLeft = Math.max(0, window.innerWidth - draggingWindow.offsetWidth), maxTop = Math.max(0, window.innerHeight - draggingWindow.offsetHeight - taskbarHeight);
    draggingWindow.style.left = Math.max(0, Math.min(event.clientX - dragOffsetX, maxLeft)) + "px";
    draggingWindow.style.top = Math.max(0, Math.min(event.clientY - dragOffsetY, maxTop)) + "px";
  });
...
function bindDesktopSelection() {
  ...
  desktop.addEventListener("mousedown", (event) => {
    ...
    selecting = true; selectionStartX = event.clientX; selectionStartY = event.clientY;
    box.style.left = selectionStartX + "px"; box.style.top = selectionStartY + "px"; box.style.width = "0px"; box.style.height = "0px"; box.style.display = "block";
    document.querySelectorAll(".desktop-icon").forEach((icon) => icon.classList.remove("selected")); closeStartMenu();
  });
  document.addEventListener("mousemove", (event) => {
    if (!selecting) return;
    const left = Math.min(selectionStartX, event.clientX), top = Math.min(selectionStartY, event.clientY), width = Math.abs(event.clientX - selectionStartX), height = Math.abs(event.clientY - selectionStartY);
    box.style.left = left + "px"; box.style.top = top + "px"; box.style.width = width + "px"; box.style.height = height + "px";
    const rect = box.getBoundingClientRect();
    document.querySelectorAll(".desktop-icon").forEach((icon) => { const iconRect = icon.getBoundingClientRect(); const overlaps = rect.left < iconRect.right && rect.right > iconRect.left && rect.top < iconRect.bottom && rect.bottom > iconRect.top; icon.classList.toggle("selected", overlaps); });
  });
  document.addEventListener("mouseup", () => { if (!selecting) return; selecting = false; box.style.display = "none"; });
}
function bindStaticControls() {
  document.addEventListener("click", (event) => { if (event.target.closest("#user-tile")) { event.preventDefault(); openPasswordDialog(); return; } });
  ...
  document.querySelectorAll(".window-close").forEach((button) => button.addEventListener("click", () => closeWindow(button.closest(".window"))));
  document.querySelectorAll(".window-close-soft").forEach((button) => button.addEventListener("click", () => closeWindow(button.closest(".window"))));
  document.querySelectorAll(".window").forEach((win) => win.addEventListener("mousedown", () => { highestZIndex += 1; win.style.zIndex = highestZIndex; }));
  document.querySelectorAll(".computer-drive-nav").forEach((button) => button.addEventListener("click", (event) => { event.stopPropagation(); switchComputerView(button.dataset.computerView); }));
  ...
  ["denmark-rant-trash", "duke-rant-trash"].forEach((id) => { const item = $(id); item?.addEventListener("click", () => selectTrashFile(item.dataset.trashId)); item?.addEventListener("dblclick", () => openWindow(item.dataset.window)); });
  $("travel-login-button")?.addEventListener("click", runTravelLogin);
  ["travel-username-input", "travel-password-input"].forEach((id) => $(id)?.addEventListener("keydown", (event) => { if (event.key === "Enter") runTravelLogin(); }));
  document.querySelectorAll(".agency-card").forEach((button) => button.addEventListener("click", () => navigateBrowser(button.dataset.page)));
  ...
  document.querySelectorAll(".orange-tab").forEach((button) => button.addEventListener("click", () => renderOrangePanel(button.dataset.orangePanel)));
...
function boot() {
  ...
  try {
    bindStaticControls(); bindWindowOpeners(); bindWindowDragging(); bindDesktopSelection();
    renderFlights(); renderOrangeDays(); renderOrangePanel("lineup"); renderCountries(); renderVehicles(); renderMsnContacts(); syncDynamicNames(); switchComputerView("root"); updateClock(); setInterval(updateClock, 1000);
  } catch (error) { console.error("Fortia boot failed", error); }
}
